...
# The trades of and benefits of different web rendering technique
...
- [How browser rendering works — behind the scenes](https://blog.logrocket.com/how-browser-rendering-works-behind-scenes/) - LogRocket Blog.
...
- [Understanding Role of Rendering Engines in Browsers](https://www.browserstack.com/guide/browser-rendering-engine) - BrowserStack.
...
- **First Input Delay** (FID): Captures the user’s first impression of a site’s interactivity and responsiveness by measuring the time from when a user first interacts with a page to the time when the browser is able to respond to that interaction. - A low FID is crucial for a page to feel responsive1.
- **Cumulative Layout Shift** (CLS): Quantifies the amount of unexpected layout shift of visible page content. - A low CLS ensures that the page is stable as it loads, preventing elements from moving around as resources are loaded1.
...
1. **Incremental Static Rendering (ISR)**:
...
2. **Island Architecture**:
   ...
   - **What it is**: This architecture suggests a compartmentalized view of a web page with static and dynamic "islands" of content⁶.
   - **How it works**: Static regions are pure HTML, while dynamic regions can rehydrate independently. This reduces the amount of JavaScript needed for interactivity⁶.
   - **Advantages**: It improves performance by reducing the JavaScript payload and allows for more efficient interactivity on web pages⁶.
